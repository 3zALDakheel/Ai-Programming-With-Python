...
def get_pet_labels(image_dir):
    ...
    for i in range(0, len(input_f), 1): 
        # Skips file if starts with . (like .DS_Store of Mac OSX) because it 
        # isn't an pet image file
        if input_f[i][0] != ".":
            # Sets string to lower case letters
            # Splits lower case string by _ to break into words 
            pet_image = input_f[i].lower().split("_")          
            ...
            if input_f[i] not in results_dic:
                results_dic[input_f[i]] = [pet_label]
              
            else:
                print("** Warning: Duplicate files exist in directory:",input_f[i]) 
